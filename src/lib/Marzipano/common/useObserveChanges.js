...
function useObserveChanges(
  items,
  eqFuncPresent = Object.is,
  eqFuncSame = null
) {
      ...
      ([draftItemsLookup, draftAdded, draftUpdated, draftDeleted]) => {
        ...
        for (const item of items) {
          const existingKey = item.key
            ? draftItemsLookup.has(item.key)
              ? item.key
              : null
            : Array.from(draftItemsLookup.keys()).find(key =>
                eqFuncPresent(draftItemsLookup.get(key), item)
              );

          const sameFunc = eqFuncSame || eqFuncPresent;
          const outdated =
            existingKey && !sameFunc(item, draftItemsLookup.get(existingKey));
